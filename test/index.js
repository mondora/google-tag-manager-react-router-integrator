...
describe("googleTagManagerInit", () => {
  ...
  it("creates a googletagmanager iframe with passed GTM_CONTAINER_ID", () => {
    ...
    expect(document.getElementById("GTM_iframe").src).to.be.equal(
      "https://www.googletagmanager.com/ns.html?id=myGTM_CONTAINER_ID"
    );
  });

  it("creates a googletagmanager iframe with correct style", () => {
    googleTagManagerInit("myGTM_CONTAINER_ID");
    expect(document.getElementById("GTM_iframe").style.display).to.equal(
      "none"
    );
    expect(document.getElementById("GTM_iframe").style.visibility).to.equal(
      "hidden"
    );
  });
  ...
  it("creates a script iframe with passed GTM_CONTAINER_ID", () => {
    ...
    expect(document.getElementById("GTM_script").src).to.be.equal(
